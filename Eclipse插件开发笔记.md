...
Composite的实际思想基于设计模式中的Composite模式，利用容器来包含其他容器或基本控件，可以将控件组合成树状的结构。一个Composite容器中可以包含任意多的基本控件或子容器控件，父容器项处理基本控件一样对子容器发送各种消息，而子容器在接收到这些消息后，再负责通知包含在其中的控件。这种层状结构有利于车技模块化的程序，能够方便的因对结构的变化，也为系统提供更好的扩展性，时几乎所有GUI设计系统都会使用的模式。
...
上面的代码创建了一个名为MyControl的控件，使用它的方法和使用基本控件一样，代码如下：
...
可以用于Composite的样式，如BORDER,NO_RADIO_GROUP和RIGHT_TO_LEFT等都可以直接应用于Group容器。下面的代码演示了如何创建Group并设定标题。
...
无论是Composite还是Group，都是创建在一个父容器里面的，那么必然会有一个顶层的容器可以不需要父容器而存在，代表窗口的Shell，就是这个最顶层的容器。Shell时Composite的子类，但是它只能最为顶层的父容器出现，而无法被嵌套在其他的容器内。在第3章已经介绍过一些Shell的知识。

创建Shell的时候，有着比创建Composite更多的样式可以选择，但是在使用这些繁多的样式时，有一点要注意，由于Shell是直接与操作系统的窗口挂钩的，这些样式的实现。都依赖于操作系统是否提供了对应的功能。如果使用了操作系统不支持的样式，就不会产生效果。即使是同样的样式，由于不同操作系统的图形平台具体的实现不同，也可能产生不同的效果。

常用的样式：

SHELL_TRIM窗口有关闭、最小化和最大化按钮，可以调整大小。由CLOSE,MIN,MAX,TITLE和RESIZE几种基本样式组成。CLOSE，MIN和MAX分别控制着显示窗口上的关闭、最小化和最大化按钮。TITLE控制着是否显示标题栏，而RESIZE则控制着窗口的大小是否能够被改变。需要注意的是，如果使用的CLOSE，MIN或MAX样式，无论是否使用TITILE样式标题栏都会显示。

DIALOG_TRIM包含了对话窗口常用的样式。它是由TITLE，CLOSE和BORDER样式组成的。利用TOOL样式创建的窗口，它的标题栏会以小于正常尺寸的样子显示出来。在程序中，这种窗口通常是作为浮动工具栏出现，这正是它被称为TOOL的原因。

无论是MAX，MIN，CLOSE还是TITLE，BORDER，RESIZE，它们的作用都是在窗口上添加装饰组件，如关闭按钮、边框等，所以这一类样式有个共同的名字，称为装饰类样式。但是NO_TRIM样式的作用却恰恰相反，会去掉所有的装饰组件，如果NO_TRIM和其他装饰类的样式共同应用于一个shell，所有装饰类的样式都会失效，Shell会显示成一个没有边框，没有标题的窗口。代码如下

```java
Shell shell = new Shell(display,SWT.SHELL_TRIM|SWT.NO_TRIM);
```

某些媒体播放器的窗口在播放视频的时候无论打开多少窗口，它都是浮在最上层。SWT提供了ON_TOP样式以实现这一功能，ON_TOP样式可以附加在任意样式的窗口上，它唯一的作用就是使窗口永远处于用户桌面的最上层。

Shell的模态（Modality）样式时一类比较特殊的样式。程序中常常有这种情况，当打开某个对话框后，再试图操作会话框的父窗口时，父窗口不做任何响应，这通常是因为对话框窗口阻截了发送到父窗口的操作消息造成的，可以通过设置Shell容器的模态样式来决定是否阻截这些操作消息。可以使用的模态样式有4种